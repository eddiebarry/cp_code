...
private:
    ...
    int node_find_rank(avl_node* n , avl_tree_dt key){
        avl_node* temp = n;
        int x = 0;
        while(temp->key!=key){
            if(temp->key<key){
                x+= size(temp)-size(temp->right_child);
...
public:
    ...
    avl_tree_dt Find_rank(avl_tree_dt key){
        int n = node_find_rank(root,key);
        return n;
    }
