...
void run_sim_mst(){
    ...
    double total_topology_calc_time = 0;
    ...
    while(flag1){
        ...
        int end = clock();
        total_topology_calc_time += (end-start)/double(CLOCKS_PER_SEC)*1000;
        //        cout<<"1 "<<endl;
        //        cout<<"1 "<<endl;
        if(num_alive <= 0.9*NUM_NODES&&flag9){
            ...
            cout<<"average topology calc time is "<<total_topology_calc_time/rounds<<"\n";
        }
    }
}
...
void run_sim(){
    ...
    run_sim_mst();
    int end = clock();
    cout << "time for complete sim: " << (end-start)/double(CLOCKS_PER_SEC)*1000 << endl;
}
