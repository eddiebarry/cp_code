...
{ 
    ...
    for (int i = 1; i < n; i++) 
    { 
        if (arr[i] < arr[tailIndices[0]] && (arr[tailIndices[0]]&arr[i]==arr[tailIndices[0]]) ) 
        { 
            // new smallest value 
            tailIndices[0] = i; 
        } 
        else if (arr[i] > arr[tailIndices[len-1]] && (arr[i]&arr[tailIndices[len-1]] == arr[tailIndices[len-1]]) ) 
        { 
